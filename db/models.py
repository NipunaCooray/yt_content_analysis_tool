...
class PilotSearchRun(Base):
    ...
    # Attribution, not ownership: losing the reviewer record should never
    # delete real collected research data -- just clear who did it.
    reviewer_id: Mapped[int | None] = mapped_column(ForeignKey("reviewers.id", ondelete="SET NULL"))
    notes: Mapped[str | None] = mapped_column(Text)
...
class PilotSearchResult(Base):
    ...
    pilot_search_run_id: Mapped[int] = mapped_column(
        ForeignKey("pilot_search_runs.id", ondelete="CASCADE"), nullable=False
    )
    # SET NULL, not CASCADE: deleting/editing a query later must never
    # discard already-collected pilot results (handover doc: "never discard
    # raw search data") -- it only loses which query originally found it.
    query_id: Mapped[int | None] = mapped_column(ForeignKey("search_queries.id", ondelete="SET NULL"))
...
class FullSearchRun(Base):
    ...
    approval_id: Mapped[int | None] = mapped_column(
        ForeignKey("search_strategy_approvals.id", ondelete="SET NULL")
    )
    parameters_json: Mapped[dict | None] = mapped_column(JSON)
    notes: Mapped[str | None] = mapped_column(Text)
...
class SearchResultRaw(Base):
    ...
    full_search_run_id: Mapped[int] = mapped_column(
        ForeignKey("full_search_runs.id", ondelete="CASCADE"), nullable=False
    )
    # SET NULL, not CASCADE -- same rationale as PilotSearchResult.query_id above.
    query_id: Mapped[int | None] = mapped_column(ForeignKey("search_queries.id", ondelete="SET NULL"))
    video_id: Mapped[str] = mapped_column(String(50), nullable=False)
    result_rank: Mapped[int] = mapped_column(Integer)
    title: Mapped[str | None] = mapped_column(Text)
...
class InformationDomainCode(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    video_coding_id: Mapped[int] = mapped_column(
        ForeignKey("video_coding.id", ondelete="CASCADE"), nullable=False
    )
